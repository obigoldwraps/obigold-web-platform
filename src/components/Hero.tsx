...
export default function Hero({ openModal }: HeroProps) {
  return (
    <section 
      ...
      style={{
        backgroundImage: `linear-gradient(rgba(10,10,10,0.4), rgba(10,10,10,0.8)), url('https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80')`,
      }}
    >
      <div className="max-w-6xl mx-auto px-6 text-center z-10 relative">
        ...
        <div className="flex flex-col lg:flex-row gap-6 justify-center mb-20">
          ...
          <button className="border-2 border-obigold-gold text-obigold-gold px-12 py-6 rounded-xl font-bold text-xl hover:bg-obigold-gold hover:text-obigold-black backdrop-blur-sm hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-500">
            VIEW GALLERY
          </button>
        </div>
      ...
      <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 animate-bounce">
        <ChevronDown className="w-8 h-8 text-white/60 hover:text-white transition-all duration-300" />
      </div>
    </section>
  );
}
